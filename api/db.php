...
class db extends mysqli {
  ...
  function insert($name, $desc, $type, $amount, $date) {
    if ($type === "") $type = "其他";
    $this->checkPass($this->query("INSERT INTO journal (`name`, `desc`, `type`, amount, `date`) VALUES ('{$name}', '{$desc}', '{$type}', {$amount}, '{$date}')"));
  ...
  function modify($id, $name, $desc, $type, $amount, $date) {
    function modifyTool($fieldName, $fieldVal, $that, $id) {
      if ($fieldVal !== "") $that->checkPass($that->query("UPDATE journal SET `{$fieldName}`='{$fieldVal}' WHERE `id`={$id}"));
    }
